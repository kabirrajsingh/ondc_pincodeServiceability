...
def process_sparse_matrix(file, bucket_name, blob_name):
    try:
        ...
        # Assuming DoubleTrie is defined/imported properly
        drie = DoubleTrie(init_company_pincode_list)
        serialized_drie = pickle.dumps(drie)
        print(type(serialized_drie))  # Debugging statement
        
        # Assuming upload_to_gcs is defined properly
        upload_to_gcs(bucket_name, blob_name, serialized_drie)
        
        return "Drie uploaded"
...
def handle_request(request):
    if request.method == 'POST':
        ...
        result = process_sparse_matrix(file_content, bucket_name, blob_name)
        ...
        response = jsonify({"error": "Only POST requests are supported."})
        # Add CORS headers
        response.headers.add("Access-Control-Allow-Origin", "*")
        response.headers.add("Access-Control-Allow-Methods", "POST")
        response.headers.add("Access-Control-Allow-Headers", "Content-Type")
